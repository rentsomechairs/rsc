...
export function initReview({ gotoAddress, gotoDone } = {}){
  ...
  btnPlace.onclick = async () => {
    ...
    try {
      // Place either 1 booking (single date) or 5 (annual deal)
      for (const d of dates2){
        ...
        await backendCreateBooking(booking);
      }

      // clear local checkout/cart regardless (UI behavior stays the same)
      const db3 = readDb();
      db3.checkout = {};
      writeDb(db3);
      clearCart();
      ...
      alert(annual2 ? '5 bookings placed!' : 'Booking placed!');
      gotoDone?.();
    } catch (e) {
      alert(e?.message || 'Booking failed.');
    } finally {
      btnPlace.disabled = false;
      btnPlace.textContent = annual2 ? 'Place 5 Bookings' : 'Place Booking';
    }
  };
